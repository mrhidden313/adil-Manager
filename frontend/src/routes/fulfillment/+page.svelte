...
<div class="flex h-screen bg-transparent text-slate-800 font-sans relative">
  ...
  <aside class="w-72 liquid-sidebar text-slate-300 flex-col hidden md:flex shadow-2xl z-10 shrink-0">
    ...
    <div class="flex-1 py-6 px-3 space-y-1.5 overflow-y-auto">
      <div class="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2 px-3">Local Operations</div>
      <button use:ripple class="w-full flex items-center space-x-3 liquid-glass-btn active px-3.5 py-2.5 rounded-xl font-medium">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-cyan-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4"/></svg>
  ...
  <main class="flex-1 flex flex-col min-w-0 overflow-hidden">
    ...
    <header class="h-16 liquid-header flex items-center justify-between px-4 md:px-8 z-20 shrink-0">
      ...
      <div class="hidden md:flex items-center gap-3">
        ...
        <h2 class="text-lg font-extrabold text-white tracking-wide">Local Agent Dashboard</h2>
      </div>
      <div class="flex items-center space-x-4">
        <div class="hidden sm:inline-flex items-center px-3 py-1 rounded-full text-xs font-black uppercase tracking-wider bg-emerald-500/15 border border-emerald-400/30 text-emerald-300 shadow-inner">
          Fulfillment Mode
        </div>
        <div class="w-9 h-9 rounded-full bg-gradient-to-tr from-emerald-400 via-cyan-400 to-indigo-500 p-[1.5px] shadow-lg hover:scale-105 transition-transform duration-300 cursor-pointer" onclick={() => showProfile = true}>
          <div class="w-full h-full rounded-full bg-slate-900 flex items-center justify-center text-white font-bold text-sm">
            L
          </div>
        </div>
        <button aria-label="Close menu" onclick={() => { localStorage.clear(); window.location.href = '/'; }} class="md:hidden text-slate-400 hover:text-white">
