...
<div class="md:hidden fixed bottom-0 left-0 w-full h-16 bg-white border-t border-slate-200 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] z-40 flex items-center justify-around px-2 pb-safe">
  ...
  {#if role === 'MANAGER'}
    ...
    <button onclick={() => window.location.href = '/manager/pads'} class="flex flex-col items-center justify-center w-full h-full text-slate-500 hover:text-indigo-600 {currentPath === '/manager/pads' ? 'text-indigo-600' : ''}">
      <svg class="w-6 h-6 mb-1 {currentPath === '/manager/pads' ? 'text-indigo-600' : ''}" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 002-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10"/></svg>
      <span class="text-[10px] font-bold">Pads</span>
    </button>
  ...
  {:else if role === 'SALES'}
    <button onclick={() => window.location.href = '/sales'} class="flex flex-col items-center justify-center w-full h-full text-slate-500 hover:text-indigo-600 {currentPath === '/sales' ? 'text-indigo-600' : ''}">
      <svg class="w-6 h-6 mb-1 {currentPath === '/sales' ? 'text-indigo-600' : ''}" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 002-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10"/></svg>
      <span class="text-[10px] font-bold">My Pads</span>
    </button>
